package cokr.xit.adds.core.auth.jwt;
...
import cokr.xit.adds.core.Constants.JwtToken;
import cokr.xit.adds.core.auth.model.TokenDTO;
import cokr.xit.adds.core.model.ResultCode;
import cokr.xit.adds.core.spring.config.properties.JwtProperties;
...
public class JwtTokenProvider implements InitializingBean {
    ...
    public Boolean validateToken(String token, LoginVO loginVO) {
        ...
        if(!username.equals(loginVO.getUserSe()+loginVO.getId())){
            throw ApiCustomException.create(ResultCode.INVALID_ACCESS_TOKEN);
        }
    ...
    private Boolean isTokenExpired(String token) {
        ...
        if(expiration.before(new Date()))  throw ApiCustomException.create(ResultCode.TOKEN_EXPIRED);
        return true;
    }
}
