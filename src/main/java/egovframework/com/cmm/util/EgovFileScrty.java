...
public class EgovFileScrty {
	...
	public static String encodeBinary(byte[] data) throws Exception {
		...
		return Arrays.toString(Base64.encodeBase64(data));
	}
     ...
     * @throws Exception
     */
    public static String encryptPassword(final String password, final String id) {
		...
		try {
			...
			throw ApiCustomException.create(nse.getMessage());
		}
	}
     ...
     * @throws Exception
     */
    public static String encryptPassword(final String data, final byte[] salt) {
		...
		try {
			...
			return new String(Base64.encodeBase64( md.digest(data.getBytes())));
		} catch (NoSuchAlgorithmException nse) {
			throw ApiCustomException.create(nse.getMessage());
		}
    }
    ...
    public static boolean checkPassword(String data, String encoded, byte[] salt) {
		...
		try {
			...
			return MessageDigest.isEqual( md.digest(data.getBytes()), Base64.decodeBase64(encoded.getBytes()));
		} catch (NoSuchAlgorithmException nse) {
			throw ApiCustomException.create(nse.getMessage());
		}
    }
